...
将全局锁优化为每队列独立细粒度锁后，perf 性能对比结果：

| 指标 | 全局锁 | 细粒度锁 | 变化 |
|------|--------|----------|------|
| do_cpu_work（有效计算） | 35.23% | 41.09% | **+5.86%** |
| pthread_cond_broadcast（唤醒开销） | 2.92% | 1.48% | **-49.3%** |
| __lll_lock_wait（锁竞争阻塞） | 0.53% | 0.33% | **-37.7%** |
| 总 CPU 周期 | 439B | 388B | **-11.7%** |

**性能数据**：perf 分析表明，细粒度锁优化后有效计算占比提升 5.9%，唤醒开销降低 49%，锁竞争阻塞减少 38%，总 CPU 周期下降 11.7%
